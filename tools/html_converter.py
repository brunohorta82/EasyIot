...
def _run(cmd: List[str]) -> None:
    subprocess.run([_resolve_cmd(cmd[0]), *cmd[1:]], cwd=ROOT, check=True, shell=sys.platform.startswith("win"))
...
def main() -> int:
    ...
    with CSS_MIN.open("w", encoding="utf-8", newline="\n") as css_out:
        subprocess.run([_resolve_cmd("uglifycss"), str(CSS_SRC)], cwd=ROOT, check=True, text=True, stdout=css_out, shell=sys.platform.startswith("win"))

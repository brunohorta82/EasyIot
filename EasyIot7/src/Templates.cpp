...
void prepareHAN()
{
    ...
    String idStr;
    config.generateId(idStr, han.name, han.driver, sizeof(han.uniqueId));
...
void prepareSHT3X()
{
    ...
    sht.driver = SHT3x_SENSOR;
    sht.delayRead = constantsConfig::climateReadDelay;
    String idStr;
    config.generateId(idStr, sht.name, sht.driver, sizeof(sht.uniqueId));

...
#endif
    if (mqttClient.connect(config.chipId, String(config.mqttUsername).c_str(), String(config.mqttPassword).c_str(), config.mqttAvailableTopic, 0, true, constantsMqtt::unavailablePayload, true))
    {
#ifdef DEBUG_ONOFRE

...
Modbus::Modbus(int receive_pin, int transmit_pin) : TasmotaSerial(receive_pin, transmit_pin, 1)
{
    mb_address = 0;
}
...
int Modbus::Begin(long speed, int stop_bits)
{
    int result = 0;
...
void Modbus::Send(uint8_t device_address, uint8_t function_code, uint16_t start_address, uint16_t register_count)
{
    uint8_t frame[8];
...
uint8_t Modbus::ReceiveBuffer(uint8_t *buffer, uint8_t register_count)
{
    uint8_t len = 0;
    ...
    while ((available() > 0) && (len < (register_count * 2) + 5) && (millis() - last < 10))
    {
        ...
        if (!len)
        { // Skip leading data as provided by hardware serial
            if (mb_address == data)
            {
                buffer[len++] = data;
            }
    ...
    if (CalculateCRC(buffer, len - 2) != crc)
    {
        return 9;
    } // 9 = crc error
...
uint8_t Modbus::Receive16BitRegister(uint16_t *value)
{
    //  0  1  2  3  4  5  6
...
uint8_t Modbus::Receive32BitRegister(float *value)
{
    //  0  1  2  3  4  5  6  7  8
